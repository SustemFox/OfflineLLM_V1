...
Java_com_example_offlinellm_llama_LlamaBridge_createContext(
        JNIEnv * env, jclass,
        jstring jpath, jint n_ctx, jint, jint n_threads) {
...
Java_com_example_offlinellm_llama_LlamaBridge_releaseContext(JNIEnv *, jclass, jlong ptr) {
    ...
    if (handle->ctx) { llama_free(handle->ctx); handle->ctx = nullptr; }
    if (handle->model) { llama_model_free(handle->model); handle->model = nullptr; }
    delete handle;
}
...
static std::string token_to_piece_str(const llama_model * model, llama_token token) {
    char buf[256];
    // try common 5-arg form with model
    int n = llama_token_to_piece(model, token, buf, sizeof(buf), false);
    if (n <= 0) return {};
...
static llama_token sample_temp(const float * logits, int n_vocab, float temperature, float top_p) {
    ...
    std::vector<std::pair<float,int>> probs; probs.reserve((size_t)n_vocab);
    float max_l = logits[0];
    for (int i = 1; i < n_vocab; ++i) if (logits[i] > max_l) max_l = logits[i];
    float sum = 0.f;
    for (int i = 0; i < n_vocab; ++i) { float p = expf((logits[i]-max_l)/temperature); probs.emplace_back(p,i); sum += p; }
    for (auto & pr : probs) pr.first /= sum;
    std::sort(probs.begin(), probs.end(), [](auto & a, auto & b){ return a.first > b.first; });
    if (top_p < 1.0f && top_p > 0.0f) {
        float cum=0.f; size_t cut=0;
        for (; cut<probs.size(); ++cut) { cum += probs[cut].first; if (cum >= top_p) break; }
        probs.resize(cut+1);
        float s2=0.f; for (auto & pr: probs) s2 += pr.first; for (auto & pr: probs) pr.first /= s2;
    }
    float r = (float)rand()/(float)RAND_MAX; float cum=0.f;
    for (auto & pr: probs) { cum += pr.first; if (r <= cum) return (llama_token)pr.second; }
    return (llama_token)probs.back().second;
}

static std::string generate_loop(ContextHandle * handle, const std::string & user, const std::string & system,
        int max_tokens, float temperature, float top_p, JNIEnv * env, jobject callback) {
    std::lock_guard<std::mutex> lock(handle->mu);
    ...
    int n_tok = llama_tokenize(handle->model, full.c_str(), (int32_t)full.size(), tokens.data(), (int32_t)tokens.size(), true, true);
    if (n_tok < 0) {
        tokens.resize((size_t)(-n_tok));
        n_tok = llama_tokenize(handle->model, full.c_str(), (int32_t)full.size(), tokens.data(), (int32_t)tokens.size(), true, true);
    }
    if (n_tok < 0) return "ERROR: tokenize failed";
    tokens.resize((size_t)n_tok);
    if (n_tok >= n_ctx - 8) return "ERROR: prompt too long for n_ctx";
    for (int i = 0; i < n_tok; ++i) {
        llama_batch batch = llama_batch_get_one(&tokens[(size_t)i], 1);
        if (llama_decode(handle->ctx, batch) != 0) return "ERROR: prompt decode failed";
    }
    ...
    int n_cur = n_tok;
    const int n_vocab = llama_n_vocab(handle->model);
    ...
    for (int step = 0; step < max_new; ++step) {
        ...
        std::string piece = token_to_piece_str(handle->model, id);
        if (!piece.empty()) {
            out += piece;
            if (callback && onToken) {
...
Java_com_example_offlinellm_llama_LlamaBridge_runInference(
        JNIEnv * env, jclass, jlong ptr, jstring jprompt, jstring jsystem,
...
Java_com_example_offlinellm_llama_LlamaBridge_runInferenceStream(
        JNIEnv * env, jclass, jlong ptr, jstring jprompt, jstring jsystem,
        jint maxTokens, jfloat temperature, jfloat topP, jobject callback) {
    if (!ptr) return;
    auto * handle = reinterpret_cast<ContextHandle *>(ptr);
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_example_offlinellm_llama_LlamaBridge_benchmark(JNIEnv * env, jclass, jlong, jint, jint) {
